    ...
    def _get_price_unit(self):
        ...
        if self.purchase_line_id and self.product_id.id == self.purchase_line_id.product_id.id:
            ...
            if line.taxes_id:
                ...
                price_subtotal = line.taxes_id.with_context(round=False).compute_all(price_subtotal, currency=line.order_id.currency_id, quantity=qty)['total_void']
                price_subtotal = float_round(price_subtotal / qty / qty, precision_digits=price_unit_prec)
            if line.product_uom.id != line.product_id.uom_id.id:
                price_subtotal *= line.product_uom.factor / line.product_id.uom_id.factor
            if order.currency_id != order.company_id.currency_id:
                ...
                price_unit = order.currency_id._convert(
                    price_subtotal, order.company_id.currency_id, order.company_id, fields.Date.context_today(self), round=False)
